...
try {
    ...
    $table = 'table_client_commandes'; // Table à synchroniser
    ...
    foreach ($localData as $row) {
        // Sauvegarder l'ID avant suppression (pour la mise à jour locale)
        $rowId = $row['num'] ?? null;
        $id_ticket = $row['id_ticket'];
        $id_caisse = $row['id_caisse'];
        // Enlever l'num pour éviter les conflits en insertion distante
        unset($row['num']);
        ...
        if ($stmtRemote->execute($row)) {
            // ✅ Mise à jour locale
            if ($rowId !== null) {
                $updateLocal = $pdoLocal->prepare("UPDATE $table SET sendserveur = 0 WHERE num = :num");
                $updateLocal->execute([':num' => $rowId]);
            }
    
            // ✅ Mise à jour distante : on utilise ici un champ unique (par exemple `code`)
