...
<!--        <input type="text" name="username" placeholder="Username" value="test" id="username" required>-->
    ...
    <div id="chooseCaisse" style="padding:20px">
        ...
        if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
            ...
            $sql = "SELECT nbcaisse FROM client WHERE id = $client_id";
            ...
            if ($numligne > 0) {
                ...
                for ($x = 1; $x <= $nbcaisse; $x++) {
                    $sql = "SELECT * FROM id_caisse_used WHERE id_caisse = $id_caisse AND client_id = $client_id";
                    $check = $con->query($sql);
                        ...
                        <div class="row" style="margin: 20px">
                            <div class="col-md-7">
                                <button type="button" class="btn btn-block btn-dark" disabled
                                        onclick="setIdCaisse('<?php echo $x; ?>')"><?php echo "Caisse n° " . $x . " utilisé." ?></button>
                            </div>
                            <div class="col-md-5">
                                <a  class="btn btn-block btn-danger"  href="../login/logout.php?id_caisse=<?php echo $id_caisse ?>&user_id_deco=<?php echo $_SESSION['user_id'] ?>"
                                >Déconnexion</a>
                            </div>
                        </div>
                        ...
                        <div class="row t">
                            <div class="col-md-8" style="margin: 20px auto">
                                <button type="button" class="btn btn-block btn-dark"
                                        onclick="setIdCaisse('<?php echo $id_caisse; ?>')"><?php echo "Caisse n° " . $id_caisse; ?></button>
                            </div>
